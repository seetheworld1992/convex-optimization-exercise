...
# H_init = 0.01 * np.identity(2)   # ヘッセ行列の係数でけっこう挙動が変わる
tol = 1e-6

(x_arr, loss_arr, eta_arr) = newton_method_algorithm(nonquad_obj, ls_obj, x_init, tol)
(x_arr_bfgs, loss_arr_bfgs, eta_arr_bfgs) = bfgs_method_algorithm(nonquad_obj, ls_obj, x_init, tol)
(x_arr_dfp, loss_arr_dfp, eta_arr_dfp) = dfp_method_algorithm(nonquad_obj, ls_obj, x_init, tol)
