...
def update_sr1(H, s, y):
    r = 1e-8
    z = s - np.dot(H, y)
    if np.abs(np.dot(y , z) ) >= r * np.linalg.norm(y, ord=2) * np.linalg.norm(z, ord=2):
        H_new = H + np.outer(z, z) / np.dot(z, y)
    else:
        H_new = H
    return H_new


def sr1_method_algorithm(objective_obj, line_search_obj, x_init, tol = 1e-1):
    ...
    minus_sr1_step = np.dot(H_init, grad_f)
    eta = line_search_obj.line_search(x_init, minus_sr1_step)
    ...
    while np.linalg.norm(grad_f, ord=2) >= tol:
        ...
        # current iteration
        x_new = x - eta * minus_sr1_step
        objective_new = objective_obj.objective_func(x_new)
        grad_f_new = objective_obj.full_path_grad(x_new)        

        # for next iteration       
        s = - eta * minus_sr1_step  # = x_new - x
        y = grad_f_new - grad_f        
        if first_time:
            Hessian_inv = np.dot(y,s) / np.dot(y,y) * H_init
            first_time = False
        Hessian_inv = update_sr1(Hessian_inv, s, y)
        x = x_new
        grad_f = grad_f_new
        minus_sr1_step = np.dot(Hessian_inv, grad_f)
        eta = line_search_obj.line_search(x, minus_sr1_step)
