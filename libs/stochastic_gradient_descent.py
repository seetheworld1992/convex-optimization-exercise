...
def stochastic_gradient_descent_algorithm(objective_obj, step_size_rule_obj, x_init, iter_end = 6000):
    """
    確率的勾配降下法(SGD)のアルゴリズム
    ...
    delta_x = objective_obj.one_path_grad(x_init, extracted_matrix, extracted_vec)
    iteration = 0
    ...
    while iteration <= iter_end:
        ...
        x = x_new
        (extracted_matrix, extracted_vec, extracted_int) = extract_row_according_to_randint_index(objective_obj.get_feat_matrix(), objective_obj.get_label_vec(), batch_size)
        delta_x = objective_obj.one_path_grad(x, extracted_matrix, extracted_vec)
