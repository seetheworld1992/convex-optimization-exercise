...
def proximal_gradient_algorithm(objective_obj, prox_operator_obj, line_search_obj, x_init, iter_end = 6000):

    """
    近接勾配法(Prox-G)のアルゴリズム
    ...
    line_search_obj : class[LineSearch の具象クラス]
        ...
        定数ラインサーチ以外の場合は 具象クラス名の末尾が ForProximal であるものを想定
    ...
    while iteration <= iter_end:        
        ...
        # current iteration
        x_new = prox_operator_obj.proximal_mapping( x - eta * objective_obj.full_path_grad(x), eta)
        objective_new = objective_obj.objective_func(x_new)

        # for next iteration
        x = x_new
        eta = line_search_obj.line_search(x)
